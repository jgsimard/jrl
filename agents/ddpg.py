...
def update_critic(actor: TrainState, critic: TrainState, batch: Batch, discount: float):
    def critic_loss_fn(critic_params):
        # deterministic actions
        next_actions = actor.apply_fn(actor.target_params, batch.next_observations)

        # twin targets
        next_q = critic.apply_fn(critic.target_params, batch.next_observations, next_actions)
...
@jax.jit
def _update(actor: TrainState, critic: TrainState, batch: Batch, tau: float, discount: float):
...
class DDPG:
    ...
    def update(self, batch: Batch):
        self.actor, self.critic, info = _update(
            self.actor, self.critic, batch, self.tau, self.discount
        )
        self.step += 1
