...
class DrQNCritic(nn.Module):
    ...
    @nn.compact
    def __call__(
        self, representations: jnp.ndarray, actions: jnp.ndarray
...
class DrQPolicy(nn.Module):
    ...
    @nn.compact
    def __call__(
        self, representations: jnp.ndarray, temperature: float = 1.0
...
def update_critic(
    ...
    def critic_loss_fn(critic_params, encoder_params):
        ...
        # twin targets
        next_q1, next_q2 = critic.apply_fn(critic.target_params, next_representations, next_actions)
    ...
    return (
        critic.apply_gradients(grads=grads_critic),
...
def _update(
    ...
    new_critic, new_encoder, critic_info = update_critic(
        key_critic,
        encoder,
        actor,
        critic,
...
class DrQ:
    def __init__(
        ...
        critic_model = DrQNCritic(hidden_dims=hidden_dims, latent_dim=latent_dim, n_critic=2)
    ...
    def sample_actions(self, observations: np.ndarray, temperature: float = 1.0) -> jnp.ndarray:
        self.rng, actions = _sample_actions(
            self.rng, self.encoder, self.actor, observations, temperature
        )
